...
### Added
- 히스토리 검색: 사이드바 상단 검색창으로 과거 세션의 제목/대화 내용 전문 검색
  (대소문자 무시, 검색어는 에디터 세션 동안 유지)
- 세션별 비용 요약: 사이드바 각 세션 항목에 출력 토큰 합계와 총 비용 표시
  (예: "2h ago · out 5.2k · $0.42"). usage 데이터가 있는 세션(v0.2.1+)만 표시.
