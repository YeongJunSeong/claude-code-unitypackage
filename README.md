...
- **해결 3 (고급): 콜백을 수동 전달** — 브라우저 주소창의 전체 URL을 복사한 뒤,
  새 PowerShell 창에서 실행 (`claude auth login`이 콜백을 기다리는 동안 실행해야 함):
  ```powershell
  Invoke-WebRequest -UseBasicParsing "http://localhost:6667/callback?code=...&state=..."
  ```
  아래처럼 `StatusCode : 200`이 나오면 CLI가 콜백을 받은 것이고, 원래 `claude auth login`
  창에 `Login successful.`이 표시되면 완료됩니다:
  ```
  StatusCode        : 200
  StatusDescription : OK
  Content           : <!DOCTYPE html><html ... Claude Code 설정이 완료되었습니다 ...
  RawContent        : HTTP/1.1 200 OK
                      Transfer-Encoding: chunked
                      ...
  RawContentLength  : 473412
  ```
